...
class Manager {
  ...
  async write() {
    ...
    const staticDirOfTheme = resolve(theme, 'static');
    let copyTask = null;
    if (fs.existsSync(staticDirOfTheme)) {
      copyTask = fs.copy(staticDirOfTheme, publicStaticDir);
    }

    if (fs.existsSync('static')) {
      if (copyTask) {
        copyTask.then(() => fs.copy('static', publicStaticDir));
      } else {
        copyTask = fs.copy('static', publicStaticDir);
      }
    }
