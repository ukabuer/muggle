...
class Page {
  ...
  async processDeps(pages) {
    ...
    this.$ = cheerio.load(xml, { xmlMode: this.data.xml });
  ...
  collectExternel() {
    ...
    const scripts = [];
    const styles = [];
    const scriptElements = this.$('script[src]');
    scriptElements.each((i, el) => {
      const src = this.$(el).attr('src');
      scripts.push(resolve(templates, src));
    });
    const styleElements = this.$('link[rel=stylesheet]');
    styleElements.each((i, el) => {
      const href = this.$(el).attr('href');
      styles.push(resolve(templates, href));
    });

    return { scripts, styles };
  }

  processExternel(scriptMap, styleMap) {
    const { templates } = this.options;
    const scriptElements = this.$('script[src]');
    scriptElements.each((i, el) => {
      const src = this.$(el).attr('src');
      const path = resolve(templates, src);
      const value = scriptMap[path];
      this.$(el).attr('src', value);
    });
    const styleElements = this.$('link[rel=stylesheet]');
    styleElements.each((i, el) => {
      const href = this.$(el).attr('href');
      const path = resolve(templates, href);
      const value = styleMap[path];
      this.$(el).attr('href', value);
    });
