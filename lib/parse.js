...
async function parse(root, config) {
  ...
  const parents = [];
  const routes = [];
  const { contentsDir, templatesDir } = config;
  let current;
  while (queue.length) {
    ...
    if (current.deps && current.deps.length) {
      ...
      const filenames = await readFilesInDir(resolve(contentsDir, current.dir));
      // handle deps
      for (let i = 0; i < filenames.length; i += 1) {
        ...
        const filepath = resolve(contentsDir, current.dir + '/' + filename);
      ...
      if (current.file) {
        const filepath = resolve(contentsDir, current.file);
        item.dataFile = filepath;
        if (parents.length) {
